...
export async function getRegistryItem(
  ...
  // Fix file paths.
  console.log(
    "[DEBUG] Before fixing file paths:",
    JSON.stringify(
      files.map((f: z.infer<typeof registryItemSchema>["files"][number]) => ({
        path: f.path,
        type: f.type,
      })),
      null,
...
function getFileTarget(file: z.infer<typeof registryItemFileSchema>) {
  ...
  if (!target || target === "") {
    ...
    console.log(
      `[DEBUG] No target found, generating target for ${fileName}, type: ${file.type}`
    )
    ...
    if (
      ...
      console.log(
        `[DEBUG] Setting target for template/component/example: ${target}`
      )
    }
    ...
    if (
      ...
      if (fileName) {
        ...
        console.log(
          `[DEBUG] Setting target for dashboard/template component: ${target}`
        )
      }
    }
  }
...
function fixFilePaths(files: z.infer<typeof registryItemSchema>["files"]) {
  ...
  console.log(
    "[DEBUG] fixFilePaths called with files:",
    JSON.stringify(
      files.map((f: z.infer<typeof registryItemSchema>["files"][number]) => ({
        path: f.path,
        type: f.type,
  ...
  console.log(
    `[DEBUG] First file path: ${firstFilePath}, dirname: ${firstFilePathDir}`
  )
  ...
  console.log(
    "[DEBUG] fixFilePaths result:",
    JSON.stringify(
      result.map((f: z.infer<typeof registryItemSchema>["files"][number]) => ({
        path: f.path,
        target: f.target,
      })),
      null,
...
export function createFileTreeForRegistryItemFiles(
  ...
  console.log(
    "[DEBUG] Creating file tree for files:",
  ...
  for (const file of files) {
    ...
    console.log(
      `[DEBUG] Processing file - Path: ${file.path}, Target: ${file.target}, Using (normalized): ${pathToUse}`
    )
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (existingNode) {
        ...
        console.log(
          `[DEBUG] Created new ${isFile ? "file" : "directory"} node: ${part}`
        )
